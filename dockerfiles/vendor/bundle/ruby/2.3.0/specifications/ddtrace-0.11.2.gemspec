# -*- encoding: utf-8 -*-
# stub: ddtrace 0.11.2 ruby lib

Gem::Specification.new do |s|
  s.name = "ddtrace"
  s.version = "0.11.2"

  s.required_rubygems_version = Gem::Requirement.new(">= 0") if s.respond_to? :required_rubygems_version=
  s.metadata = { "allowed_push_host" => "https://rubygems.org" } if s.respond_to? :metadata=
  s.require_paths = ["lib"]
  s.authors = ["Datadog, Inc."]
  s.bindir = "exe"
  s.date = "2018-02-02"
  s.description = "ddtrace is Datadog\u{2019}s tracing client for Ruby. It is used to trace requests\nas they flow across web servers, databases and microservices so that developers\nhave great visiblity into bottlenecks and troublesome requests.\n"
  s.email = ["dev@datadoghq.com"]
  s.homepage = "https://github.com/DataDog/dd-trace-rb"
  s.licenses = ["BSD-3-Clause"]
  s.required_ruby_version = Gem::Requirement.new(">= 1.9.1")
  s.rubygems_version = "2.5.1"
  s.summary = "Datadog tracing code for your Ruby applications"

  s.installed_by_version = "2.5.1" if s.respond_to? :installed_by_version

  if s.respond_to? :specification_version then
    s.specification_version = 4

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<msgpack>, [">= 0"])
      s.add_development_dependency(%q<rake>, ["~> 10.5"])
      s.add_development_dependency(%q<rubocop>, ["= 0.49.1"])
      s.add_development_dependency(%q<rspec>, ["~> 3.0"])
      s.add_development_dependency(%q<rspec-collection_matchers>, ["~> 1.1"])
      s.add_development_dependency(%q<minitest>, ["= 5.10.1"])
      s.add_development_dependency(%q<appraisal>, ["~> 2.2"])
      s.add_development_dependency(%q<yard>, ["~> 0.9"])
      s.add_development_dependency(%q<webmock>, ["~> 2.0"])
      s.add_development_dependency(%q<builder>, [">= 0"])
      s.add_development_dependency(%q<addressable>, ["~> 2.4.0"])
      s.add_development_dependency(%q<redcarpet>, ["~> 3.4"])
      s.add_development_dependency(%q<pry>, ["~> 0.10.4"])
      s.add_development_dependency(%q<pry-stack_explorer>, ["~> 0.4.9.2"])
    else
      s.add_dependency(%q<msgpack>, [">= 0"])
      s.add_dependency(%q<rake>, ["~> 10.5"])
      s.add_dependency(%q<rubocop>, ["= 0.49.1"])
      s.add_dependency(%q<rspec>, ["~> 3.0"])
      s.add_dependency(%q<rspec-collection_matchers>, ["~> 1.1"])
      s.add_dependency(%q<minitest>, ["= 5.10.1"])
      s.add_dependency(%q<appraisal>, ["~> 2.2"])
      s.add_dependency(%q<yard>, ["~> 0.9"])
      s.add_dependency(%q<webmock>, ["~> 2.0"])
      s.add_dependency(%q<builder>, [">= 0"])
      s.add_dependency(%q<addressable>, ["~> 2.4.0"])
      s.add_dependency(%q<redcarpet>, ["~> 3.4"])
      s.add_dependency(%q<pry>, ["~> 0.10.4"])
      s.add_dependency(%q<pry-stack_explorer>, ["~> 0.4.9.2"])
    end
  else
    s.add_dependency(%q<msgpack>, [">= 0"])
    s.add_dependency(%q<rake>, ["~> 10.5"])
    s.add_dependency(%q<rubocop>, ["= 0.49.1"])
    s.add_dependency(%q<rspec>, ["~> 3.0"])
    s.add_dependency(%q<rspec-collection_matchers>, ["~> 1.1"])
    s.add_dependency(%q<minitest>, ["= 5.10.1"])
    s.add_dependency(%q<appraisal>, ["~> 2.2"])
    s.add_dependency(%q<yard>, ["~> 0.9"])
    s.add_dependency(%q<webmock>, ["~> 2.0"])
    s.add_dependency(%q<builder>, [">= 0"])
    s.add_dependency(%q<addressable>, ["~> 2.4.0"])
    s.add_dependency(%q<redcarpet>, ["~> 3.4"])
    s.add_dependency(%q<pry>, ["~> 0.10.4"])
    s.add_dependency(%q<pry-stack_explorer>, ["~> 0.4.9.2"])
  end
end
